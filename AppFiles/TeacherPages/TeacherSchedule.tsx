...
function TeacherSchedule({route, navigation}: {route: any; navigation: any}) {
  ...
  const [selectedYear, setYear] = useState(todayYear);
            ...
            for (const idx in snapshot5.val().classes) {
              ...
              for (const idxWeekDay in validWeekDays) {
                ...
                for (const idxTimePeriod in currentClassHours[weekDay]) {
                  if (
                    snapshot5.val().Name !==
                    currentClassHours[weekDay][idxTimePeriod].teacher_name
                  ) {
                    continue;
                  }
                  ...
                  finalSchedule[weekDay][idxTimePeriod] = {
                    subject_name:
                      currentClassHours[weekDay][idxTimePeriod].subject_name,
                    class_location:
                      currentClassHours[weekDay][idxTimePeriod].class_location,
                    className: currClass,
                  };
                }
              }
